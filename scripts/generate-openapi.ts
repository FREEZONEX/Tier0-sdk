...
function generateApi(swagger: SwaggerDoc): string {
  ...
  code += `import { normalizeNotificationRequest } from './notifications.js';\n`;
  ...
  for (const [moduleName, fns] of Object.entries(modules)) {
    ...
    for (const operation of fns) {
      ...
      const bodyArg = operation.bodyType || needsBody(operation)
        ? `, ${operation.path === '/openapi/v1/notifications/send' && operation.method === 'post' ? `normalizeNotificationRequest(${bodyExpression})` : bodyExpression}`
        : '';
