...
| `sender` | Agent | Filled from the calling app's identity, see send.md. For an `app` sender, `name` is the App's own name — it is the fallback shown when the platform's app lookup misses, not a free label |
