...
>
> 统计范围是受限例外：当 Dashboard 等固定产品能力必须独立读取某个聚合范围的 retained Current 和有界历史时，可以按 [`../../references/app-statistics-design.md`](../../references/app-statistics-design.md) 为**预声明、数量有硬上限**的稳定范围建 topic。路径段必须使用不含 `/` 的人类可读稳定 slug，不能使用 UUID、数据库主键或用户查询值；Topic manifest 必须列出范围并声明正整数 `maxStatisticScopes`，新增范围必须经过 manifest 变更，达到上限后拒绝创建，禁止运行时随目录记录无限建 topic。该例外只适用于聚合范围，不适用于订单、客户、设备等普通业务实体。
>
